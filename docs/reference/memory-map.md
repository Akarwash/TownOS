...
| Ring-3 program code (`.user_text`) | `0x400000` (4M) | three small functions | `linker.ld`, `user/user_program.c` | The LINKED image. Holds `user_program_a`, `_b`, and `_c`. Its own `PT_LOAD` segment so the gap from the kernel is not padded into the file. In the boot tree it is a shared PD[2] huge page; with per-process paging each task instead gets a PRIVATE 4KB copy at this same virtual address (see below). |
